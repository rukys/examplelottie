...
const App = () => {
  ...
  const renderItem = ({item}) => (
    <View style={[tw`items-center w-full px-6`, {width}]}>
      <LottieView
        source={item.animation}
        autoPlay
        loop
        style={tw`mt-10 h-96 w-96`}
      />
      <Text
        style={tw`mt-8 mb-4 text-xl text-center text-black font-montserratSemiBold`}>
        {item.title}
      </Text>
      <Text style={tw`text-base text-center text-black font-montserrat mb-14`}>
        {item.subtitle}
      </Text>
    </View>
  );
    ...
    <SafeAreaProvider>
      <SafeAreaView style={tw`flex-1 bg-white`}>
        ...
        <View style={tw`flex-row justify-center mb-6`}>
          {slides.map((_, index) => {
            ...
            return (
              <Animated.View
                key={index}
                style={[tw`w-2 h-2 mx-1 rounded-full bg-green`, {opacity}]}
              />
            );
          })}
        </View>
        ...
        <View style={tw`px-6 mb-6`}>
          <TouchableOpacity
            style={tw`items-center justify-center w-full h-10 rounded-lg bg-green`}
            onPress={handleNext}>
            <Text style={tw`text-white font-montserratSemiBold`}>
